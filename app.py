...
HF_REPO_ID = "gangurde/cattle_disease_model"
MODEL_FILES = {
...
def load_model(animal_type):
    ...
    if models.get(animal_type) is not None:
        return models[animal_type]
    
    try:
        print(f"Loading {animal_type} disease model...")
        ...
        # Try to download from Hugging Face if available
        if HF_HUB_AVAILABLE:
            try:
        ...
        print(f"✗ Error loading {animal_type} model: {e}")
        raise Exception(f"Failed to load {animal_type} disease detection model: {str(e)}")

print("YOLO models configured for lazy loading from Hugging Face")
print(f"Repository: {HF_REPO_ID}")
print("Models will be downloaded on first use to prevent deployment timeout")
